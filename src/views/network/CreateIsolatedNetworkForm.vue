  ...
  methods: {
    ...
    fetchNetworkOfferingData (forVpc) {
      ...
      api('listNetworkOfferings', params).then(json => {
        const listOfferings = json.listnetworkofferingsresponse.networkoffering
        this.networkOfferings = this.networkOfferings.concat(listOfferings)
      }).finally(() => {
    ...
    fetchVpcData () {
      ...
      api('listVPCs', params).then(json => {
        const listVpcs = json.listvpcsresponse.vpc
        this.vpcs = this.domains.concat(listVpcs)
      }).finally(() => {
