...
export default {
  ...
  methods: {
    ...
    async filterTabs () {
      ...
      // Regular guest networks with Source Nat only have Firewall & VPN
      if (this.resource && !this.resource.vpcid && this.resource.issourcenat) {
        this.tabs = this.defaultTabs.concat(this.$route.meta.tabs.filter(tab => ['firewall', 'vpn'].includes(tab.name)))
        return
      }
